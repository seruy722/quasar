...
//        return response(['asc' => date("Y-m-d H:i:s", strtotime($request->created_at))]);
    ...
    public function getClientStorehouseData()
    {
        ...
        if (auth()->user()->code_id) {
            array_push($ids, auth()->user()->code_id);
        } else if (!auth()->user()->code_id) {
            $ids = json_decode(auth()->user()->code_ids);
        }
        $data = $this->storehouseDataList(1)->whereIn('storehouse_data.code_client_id', $ids)->where('storehouse_data.fax_id', 0)->get();
        ...
        $data2 = $this->storehouseDataList(1)->whereIn('storehouse_data.code_client_id', $ids)->whereIn('storehouse_data.fax_id', $fIds)->get();
