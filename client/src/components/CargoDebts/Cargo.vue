...
export default {
  ...
  computed: {
    ...
    titleTable() {
      return `Сумма: ${_.round(_.sumBy(this.cargo, "sum"))}, Скидки: ${_.round(
        _.sumBy(this.cargo, "sale")
      )}`;
    },
  },
  methods: {
    ...
    async viewEditDialog(data, event) {
      if (!_.includes(_.get(event, "target.classList"), "select_checkbox")) {
        ...
        if (data.row.type) {
          ...
          !data.row.type &&
          !_.includes(event.target.classList, "faxName") &&
          _.has(data, "row.arr")
        ) {
          const { setFormatedDate } = await import(
            "src/utils/FrequentlyCalledFunctions"
          );
          this.entryData = setFormatedDate(
            _.cloneDeep(_.get(data, "row.arr")),
            ["created_at"]
          );
