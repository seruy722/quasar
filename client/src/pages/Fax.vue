...
<script>
    ...
    export default {
        ...
        watch: {
            ...
            search(val) {
                if (_.trim(val)) {
                    this.faxSideData = this.searchInList(val);
                } else {
                    this.faxSideData.push(...this.faxTableData);
                }
            },
        },
        mounted() {
        ...
        methods: {
            searchInList(val) {
                ...
                return this.faxTableData.filter((v) => v.category_name.toLowerCase()
                ...
                Promise.all([getStorehouseTableData(this.$store), getFaxes(this.$store)])
                  .then(() => {
                      ...
                      if (_.isEmpty(this.storehouseSideData)) {
                          this.storehouseSideData.push(...this.storehouseData);
                      }
            ...
            // Действие, когда пользователь закончил сдвиг элемента в лево
            onLeft(item) {
