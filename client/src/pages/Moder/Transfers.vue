...
export default defineComponent({
  ...
  setup() {
    ...
    const allTransfers = computed(() => store.getters['transfers/getTransfers']);
    ...
    watch(
        () => clientCodes,
        (val) => {
          _.set(transferData, 'client_id.options', val);
        },
        {
          immediate: true,
        },
    ...
    watch(
        ...
        (val) => {
          if (val > 2) {
            _.set(transferData, 'issued_by.require', true);
          } else {
            _.set(transferData, 'issued_by.require', false);
            _.set(transferData, 'issued_by.value', null);
          }
        },
        {
          immediate: true,
    ...
    function refresh() {
      loading.value = true;
      return store.dispatch('transfers/fetchTransfers')
          .finally(() => {
            loading.value = false;
          });
    }
    ...
    function viewEditDialog(val, event) {
      if (!_.includes(_.get(event, 'target.classList'), 'select_checkbox')) {
        if (val) {
          ...
          $q.loading.show();
          Promise.all([getClientCodes(store)])
              .then(() => {
                openCloseDialog(true);
                $q.loading.hide();
              });

          _.forEach(transferData, (item) => {
