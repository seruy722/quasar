...
export const numberFormat = (number) =>
  new Intl.NumberFormat("ru-RU").format(_.toNumber(number));
...
export const statusColor2 = (value) => {
  const findLabel =
    _.find(getFromSettings("transferStatus"), { value }) ||
    _.find(getFromSettings("transferStatus"), { label: value });
  return _.get(findLabel, "color");
};
export const numberUnformat = (number) =>
  _.chain(number).toString().split(" ").join("").toNumber();
